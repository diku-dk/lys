...
--   and the `x`/`y` how much it was moved.
--
-- * `#wheel {x, y}`: The mouse wheel has been used.  Note that there can
--   be multiple wheels; this is why the 'x' direction also makes
--   sense.  In most cases, however, only the `y` will be non-zero.
type event = #step f32
             | #keydown {key:i32}
             | #keyup {key:i32}
